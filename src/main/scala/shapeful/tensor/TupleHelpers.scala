...
/**
  * Contains basic helper functions for working with tuples
  * (mostly on the type level)
  */
object TupleHelpers:
  ...
  type Remove[A, B <: Tuple] <: Tuple = B match
    case EmptyTuple => EmptyTuple
    case A *: EmptyTuple => EmptyTuple  // If removing the only element, return empty
    case A *: tail => tail              // If removing first element, return tail
    case head *: tail => head *: Remove[A, tail]  // Otherwise, keep head and recurse
  ...
  type S[N <: Int] <: Int = N match
    ...
    case 5 => 6  case 6 => 7  case 7 => 8  case 8 => 9  case 9 => 10
    case 10 => 11  case 11 => 12  case 12 => 13  case 13 => 14  case 14 => 15
  ...
  def createTupleFromSeq[T <: Tuple](seq: Seq[Int]): ToIntTuple[T] = {
    require(seq.length <= 6, s"Tuple size ${seq.length} not supported, maximum is 6")
    ...
    seq.length match {
      ...
      case 5 => (seq(0), seq(1), seq(2), seq(3), seq(4)).asInstanceOf[ToIntTuple[T]]
      case 6 => (seq(0), seq(1), seq(2), seq(3), seq(4), seq(5)).asInstanceOf[ToIntTuple[T]]
    }
  }
