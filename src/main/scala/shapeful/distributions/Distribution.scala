...
/** Base trait for all probability distributions
  *
  * @tparam Support
  *   The type of values this distribution generates
  */
...
/** Factorized distribution over independent elements
  *
  * Represents a distribution over a tensor where each element is an independent random variable with its own
  * parameters. This is the product distribution: p(x) = ∏ᵢ p(xᵢ)
  *
...
trait FactorizedDistribution[S <: Tuple] extends Distribution[Tensor[S]]:
  ...
  /** Joint log probability under independence assumption
    *
    * Returns the sum of element-wise log probabilities, exploiting the independence: log p(x) = Σᵢ log p(xᵢ)
    */
...
/** Multivariate distribution over correlated dimensions
  *
  * Represents a joint distribution over d-dimensional vectors where dimensions may be correlated. Returns scalar log
