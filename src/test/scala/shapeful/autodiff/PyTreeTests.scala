...
class PyTreeTests extends FunSuite:
  ...
  test("derives ToPyTree for Tensor1") {
    val tensor = Tensor1(Axis["Label"], Seq(1f, 2f, 3f))
  ...
  test("derives ToPyTree for Tensor2") {
    val tensor = Tensor2(Axis["Label1"], Axis["Label2"], Seq(Seq(1f, 2f), Seq(3f, 4f)))
  ...
  test("derives ToPyTree for Tuple of different tensors") {
    val tensor1 = Tensor1(Axis["Label1"], Seq(1f, 2f))
    val tensor2 = Tensor2(Axis["Label2"], Axis["Label3"], Seq(Seq(3f, 4f), Seq(5f, 6f)))
  ...
  test("derives ToPyTree for case class with tensors") {
    ...
    val params = SomeParams(
      Tensor2(Axis["Label1"], Axis["Label2"], Seq(Seq(1f, 2f), Seq(3f, 4f))),
      Tensor1(Axis["Label2"], Seq(5f, 6f))
    )
  ...
  test("derives ToPyTree for nested case classes with tensors") {
    ...
    val outerParams = OuterParams(innerParams, Tensor1(Axis["Label2"], Seq(7f, 8f)))
