...
object BayesianLinearRegression extends App:
    ...
    val baseDistribution = MVNormal.standardNormal[Latent](Shape1(3))// Tensor.zeros(Shape1(3)), Tensor.eye(Shape2(3, 3)) * Tensor0(0.1f)) // 3D latent space for weight, bias, logSigma
    ...
    val trajectory = optimizer.optimize(grad,initialParams).zipWithIndex.map{
        (params, iter) => {
            if iter % 50 == 0 then
