...
def tensorAPI(): Unit =
  ...
  println((ABCD.shape, py.eval("abcd.shape")))
  {
    ...
    /** REARRANGE Analog to einops rearrange, but with named axes. For JAX this replaces `transpose` and `reshape`
      * operations.
      */
    ...
    opBlock("rearrange ABCD flatten A and B") {
      py.exec("res = einops.rearrange(abcd, 'a b c d -> (b a) c d')") // einops variant
      py.exec(
        "res = jnp.reshape(abcd.transpose((1, 0, 2, 3)), (abcd.shape[0]*abcd.shape[1], abcd.shape[2], abcd.shape[3]))"
      ) // pure JAX variant
      ABCD.rearrange(
    ...
    opBlock("rearrange ABCD unflatten AB") {
      ...
      py.exec(
        "res = jnp.reshape(tmp, (abcd.shape[0], abcd.shape[1], tmp.shape[1], tmp.shape[2])).transpose((1, 0, 2, 3))"
      ) // pure JAX variant
    ...
    /** APPEND AXIS Analog to jnp.expand_dims / None indexing in JAX, adds a new axis at the end or beginning. If axis
      * must be inserted at a specific position use `rearrange` after `appendAxis` or `prependAxis`.
      */
    // AB[:, :, None]
    ...
    opBlock("vmap/zipvmap AB AC AB AC") {
      py.exec(
        "res = jax.vmap(lambda abi, aci, ab2i, ac2i: jnp.sum(abi) + jnp.sum(aci) + jnp.sum(ab2i) + jnp.sum(ac2i))(ab, ac, ab, ac)"
      )
