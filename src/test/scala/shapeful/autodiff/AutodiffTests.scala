...
class AutodiffTests extends FunSuite {
  ...
  test("diff of quadratic function1 of a scalar") {
    ...
    for i <- -10 until 10 do
        val x = Tensor(Shape.empty, i.toFloat, requiresGrad = true)
  ...
  test("diff of function2 of two scalars") {
    ...
    for i <- -10 until 10; 
        j <- -10 until 10 do
        val x : Tensor0 = Tensor(Shape.empty, i.toFloat, requiresGrad = true)
        val y : Tensor0 = Tensor(Shape.empty, j.toFloat, requiresGrad = true)
        val (dx, dy) = df((x, y))
    ...
    test("diff of function2 of two vectors") {
        ...
        val f = (x : Tensor1["Data"], y : Tensor1["Data"]) => 
            val m = x.add(y.mul(Tensor(Shape.empty, 2f, requiresGrad = false)))
            m.sum["Data"]

        val df = Autodiff.deriv(f)


        val (dx, dy) = df((Tensor.fromSeq(shape, Seq(1,1), requiresGrad = true), Tensor.fromSeq(shape, Seq(2,2), requiresGrad = true)))
