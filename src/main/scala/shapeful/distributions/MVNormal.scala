...
/** Multivariate Normal distribution
  * 
  * Models a joint probability distribution over d-dimensional vectors.
  * Unlike Normal which treats each dimension independently, MVNormal
  * captures correlations between dimensions through the covariance matrix.
  * 
  * Uses JAX's scipy.stats.multivariate_normal for efficient computation.
  * 
  * @param mu Mean vector of shape [d]
  * @param cov Covariance matrix of shape [d, d] (must be positive definite)
  */
class MVNormal[L <: Label](val mu: Tensor1[L], val cov: Tensor2[L, L])
  extends MultivariateDistribution[L]:
  ...
  /** Log probability density of a single vector
    * 
    * Returns a scalar log probability for the input vector under
    * the joint distribution.
    * 
    * @param x A single d-dimensional vector
  ...
  /** Sample a single vector from the distribution 
    ...
    * @return d-dimensional sample vector
    */
