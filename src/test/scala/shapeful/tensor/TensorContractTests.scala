...
class TensorContractTests extends FunSuite:
  ...
  test("contract: matrix-vector multiplication") {
    ...
    val matrix = Tensor2[Rows, Features](
      Seq(Seq(1.0f, 2.0f), Seq(3.0f, 4.0f))
    )

    // Vector: (Features,) = [5, 6]
    val vector = Tensor1[Features](Seq(5.0f, 6.0f))
    ...
    assertEquals(result.shape.size, 2)

    val expected = Tensor1[Rows](Seq(17.0f, 39.0f))
    assert(
  ...
  test("contract: matrix-matrix multiplication") {
    ...
    val m1 = Tensor2[Rows, Inner](
      Seq(Seq(1.0f, 2.0f), Seq(3.0f, 4.0f))
    )

    // m2: (Inner, Cols) = [[5, 6], [7, 8]]
    val m2 = Tensor2[Inner, Cols](
      Seq(Seq(5.0f, 6.0f), Seq(7.0f, 8.0f))
    )
    ...
    //         = [[19, 22], [43, 50]]
    assertEquals(result.shape.dims, Seq(2, 2))

    val expected = Tensor2[Rows, Cols](
      Seq(Seq(19.0f, 22.0f), Seq(43.0f, 50.0f))
    )
    assert(
      result.approxEquals(expected, tolerance = 1e-5f),
  ...
  test("contract: batched operations") {
    ...
    val batched = Tensor3[Batch, Rows, Inner](
      Seq(
    ...
    val weights = Tensor2[Inner, Cols](
      Seq(Seq(1.0f, 2.0f), Seq(3.0f, 4.0f))
    )
  ...
  test("contract: dot product (scalar result)") {
    // v1: (Dim,) = [1, 2, 3]
    val v1 = Tensor1[Dim](Seq(1.0f, 2.0f, 3.0f))

    // v2: (Dim,) = [4, 5, 6]
    val v2 = Tensor1[Dim](Seq(4.0f, 5.0f, 6.0f))

    // Contract over Dim, returns scalar
    val result: Tensor0 = v1.contract(Axis[Dim], v2)

    // Expected: 1*4 + 2*5 + 3*6 = 4 + 10 + 18 = 32
    assertEquals(result.shape.dims, Seq.empty)
    assertEqualsDouble(result.toFloat.toDouble, 32.0, 1e-5)
  }

  test("contract: vector-matrix multiplication") {
    // vector: (Inner,) = [1, 2, 3]
    val vector = Tensor1[Inner](Seq(1.0f, 2.0f, 3.0f))

    // matrix: (Inner, Cols) = [[1, 2], [3, 4], [5, 6]]
    val matrix = Tensor2[Inner, Cols](
      Seq(Seq(1.0f, 2.0f), Seq(3.0f, 4.0f), Seq(5.0f, 6.0f))
    )
    ...
    val expected = Tensor1[Cols](Seq(22.0f, 28.0f))
    assert(
      result.approxEquals(expected, tolerance = 1e-5f),
  ...
  test("contract: outer product becomes inner product when contracted") {
    // v1: (I,) = [1, 2]
    val v1 = Tensor1[I](Seq(1.0f, 2.0f))

    // v2: (I,) = [3, 4]
    val v2 = Tensor1[I](Seq(3.0f, 4.0f))
  ...
  test("contract: 3D tensor with 2D tensor") {
    ...
    val expected = Tensor3[Batch, I, K](
      Seq(
  ...
  test("contract: consistency with existing matmul for 2D tensors") {
    ...
    val m2 = Tensor2[Inner, Cols](
      Seq(Seq(7.0f, 8.0f), Seq(9.0f, 10.0f), Seq(11.0f, 12.0f))
    )
  ...
  test("contract: consistency with dot for 1D tensors") {
    ...
    val v1 = Tensor1[Dim](Seq(1.0f, 2.0f, 3.0f, 4.0f))
    val v2 = Tensor1[Dim](Seq(5.0f, 6.0f, 7.0f, 8.0f))
  ...
  test("contract: different batch sizes retained in result") {
    ...
    val t1 = Tensor2[Batch, Inner](
      Seq(Seq(1.0f, 2.0f), Seq(3.0f, 4.0f), Seq(5.0f, 6.0f)) // 3 batches
    )

    val t2 = Tensor2[Inner, Features](
    ...
    assertEquals(result.shape.dims, Seq(3, 3))

    // Verify first batch: [1*1 + 2*4, 1*2 + 2*5, 1*3 + 2*6] = [9, 12, 15]
    val expected = Tensor2[Batch, Features](
      Seq(
