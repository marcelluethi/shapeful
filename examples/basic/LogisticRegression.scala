...
object LogisticRegression:
  ...
  def main(args: Array[String]): Unit =
    ...
    def standardize(t: Tensor2[Sample, Feature]): Tensor2[Sample, Feature] =
      val mean = t.vmap[VmapAxis = Feature](_.mean)
      val std = t.zipVmap[VmapAxis = Feature](mean)((x, m) => (x - m).pow(Tensor0(2f)).mean.sqrt + Tensor0(1e-6f))
      t.vmap[VmapAxis = Sample]((x) => (x - mean) / std)
    ...
    def loss(p: Params): Tensor0 =
      val losses = trainingData.zipVmap[VmapAxis = Sample](labels)((sample, label) =>
